"""Runs the pretraining stage for market and cointegration-related modules."""
        ...
        if len(d_list) >= args.batch_pre or d == evalrange[-1] and len(d_list) >= 3:
            if args.pre_type == "Cointegration":
                ...
                for b in range(id_out_rand.shape[0]):
                    u_oldnow = torch.index_select(
                        u_oldlist[b], 0, id_out_rand.to(device)[b, :]
                    )
                ...
                with torch.no_grad():
                    for b in range(id_out_rand.shape[0]):
                        u_old__ = u_oldlist[b].index_put_(
                            [id_out_rand.to(device)[b, :]], u[b, :].detach()
                        )
                        u_oldlist[b] = u_old__
                ...
                x_out_all, y__, addi_x, id_out = model_pretrain.stk_gen_normal(
                    query, d_list[0], stk_id_dic, stk_dic_class
                )
...
def main(
    ...
    if args.pre_type == "market":
        ...
        if args.model_path_pre is not None:
            ...
            model = model_pretrain.stk_classification_att1(
                input_size=input_size,
                drop_out=args.dropout_g,
                stk_total=len(stk_dic_class_load.stk_dic.keys()) + 2,
                use_stk=use_stk,
            )
            model.load_state_dict(state["para"], strict=False)
            model.reinitial_stk(
                input_size=input_size, stk_total=len(stk_dic_class.stk_dic.keys()) + 2
            )
            model.use_stk = 1
        else:
            model = model_pretrain.stk_classification_att1(
    ...
    if args.pre_type == "Cointegration":
        ...
        if args.pre_type == "Cointegration":
            model = model_pretrain.stk_pred_small_2(
                stk_total=len(stk_dic_class.stk_dic.keys()) + 2, drop_out=args.dropout_g
            )
            if args.model_path_pre is not None:
                state = torch.load(args.model_path_pre, map_location=device)
                stk_dic_class = model_pretrain.stk_dic()
    ...
    if args.use_Adam:
        ...
        model_opt = torch.optim.SGD(
            params=para_list, lr=args.learning_rate, momentum=0.9
        )
        logger.info("Using SGD Optimizer")
    logger.info(
        f"train period: {query.num2day[train_range[0]]} to {query.num2day[train_range[-1]]},"
        f" length is {len(train_range)}"
    )
    logger.info(
        f"valid period: {query.num2day[valid_range[0]]} to {query.num2day[valid_range[-1]]},"
        f" length is {len(valid_range)}"
    )
    logger.info(
        f"test period: {query.num2day[test_range[0]]} to {query.num2day[test_range[-1]]},"
    ...
    if args.pre_type == "market":
        ...
        for d in range(0, len(train_range)):
            x_out_all, y__, addi_x, id_out = model_pretrain.stk_gen_normal(
                query, train_range[d], stk_id_dic, stk_dic_class
            )
    ...
    for epoch in range(args.epoch):
        ...
        for d in range(0, len(train_range_other)):
            ...
            if len(d_list) >= args.batch_pre:
                ...
                if args.pre_type == "Cointegration":
                    ...
                    price, price_pred = model(
                        addi_x_out.to(device), id_out_rand.to(device)
                    )
                    loss1 = mse_loss(price, price_pred)
                    u = price_pred - price
                    u = u.squeeze(2)
                    ...
                    for b in range(id_out_rand.shape[0]):
                        ...
                        rhonow = torch.index_select(
                            rho, 0, id_out_rand.to(device)[b, :]
                        )
                        diff = u[b, :] - u_oldnow * rhonow
                        square_diff = torch.pow(diff, 2)
                        loss2 = loss2 + square_diff.mean()
                    ...
                    total_embed = model(x_out_all.to(device), id_out.to(device))
                    label_day = day_return_type(
                        y__, return_low, return_high, percent_stock
                    )
                    ...
                    if label_day == 0:
                        ...
                        list_threetypes[2] += 1
                    label_day = torch.ones([1], dtype=torch.long).to(device) * int(
                        label_day
                    )
        ...
        valid_score, val_datalist = evaluation_pre(
            model_list[0],
            model_list[1],
            model_list[2],
            val_datalist,
        ...
        test_score, test_datalist = evaluation_pre(
            model_list[0],
            model_list[1],
            model_list[2],
            test_datalist,
        ...
        if score_va > best_val_loss:
            if args.pre_type == "market":
                print(
                    "len va",
                    sum([x[0].shape[0] for x in val_datalist]),
                    "len te",
                    sum([x[0].shape[0] for x in test_datalist]),
                )
            state_ = {
                "para": model_list[0].state_dict(),
                "stk_dic": stk_dic_class.stk_dic,
                "para_2": model_list[1].state_dict(),
            }
            torch.save(
                state_, os.path.join(work_dir, f"best_modelpre_{args.pre_type}.pkl")
            )
        ...
        if args.pre_type == "Cointegration":
            ...
            logger.info(
                f"{args.pre_type},va_score :{valid_score};te_score :{test_score}"
            )
        logger.info(
            f"model_path_pre:"

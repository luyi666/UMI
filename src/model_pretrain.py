"""Defines pretraining modules for market representation and cointegration signals."""
...
def batch_stk_gen(
    query, d_list, stk_id_dic, stk_dic_class, type="market2", days=20, sample_range=None
):
    data_dic = {}
    min_sktk = 5000
    for d in d_list:
    ...
    if type == "Cointegration":
        ...
        for d in d_list:
            ...
            addi_x_list.append(addi_x.unsqueeze(0))
            id_list.append(id_ten.unsqueeze(0))
        x_out, y_out, addi_x_out, id_out = (
            torch.cat(x_list, dim=0),
            torch.cat(y_list, dim=0),
            torch.cat(addi_x_list, dim=0),
    ...
    if type == "market2":
        x1_list, x2_list, y_list, addi_x1_list, addi_x2_list, id1_list, id2_list = (
            [],
            [],
            [],
            [],
        ...
        for dd in d_list:
            if dd != d:
                ...
                not_in_id_ind = torch.isin(
                    id_ten_, id1_ten, assume_unique=True, invert=True
                )
        ...
        x1_out, x2_out, addi_x1_out, addi_x2_out, id1_out, id2_out = (
            torch.cat(x1_list, dim=0),
            torch.cat(x2_list, dim=0),
            torch.cat(addi_x1_list, dim=0),
            torch.cat(addi_x2_list, dim=0),
            torch.cat(id1_list, dim=0),
            torch.cat(id2_list, dim=0),
        )

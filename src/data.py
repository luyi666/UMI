"""Loads parquet market data and converts it into tensors for model training."""
...
class Query:
    def __init__(self, data_dir: str, input_len: int = 1, args=None):
        self.args = args
        if args.extra_data_dir is not None:

            extra_data = pd.read_parquet(args.extra_data_dir)
            extra_data["Date"] = extra_data["Date"].apply(
        ...
        for date in dates[input_len:]:
            ...
            logger.info(
                f"date is {self.num2day[d]}, number of stocks is {len(target_list)}"
            )
    ...
    def concat2input(
        self, date_code: int, stock_list: List[int], scaler=preprocessing.StandardScaler
    ):
        result = []
        myScaler = scaler()
    ...
    def one_step(self, begin: int, stock_list: List[int] = None):
        ...
        if self.args.extra_data_dir is not None:
            if self.num2day[begin] not in self.extra_datad.keys():
                logger.error(
                    f"illegal extra_datad date_code: {begin}, date is {self.num2day[begin]}"
                )
    ...
    def one_step_tensor(self, begin: int):

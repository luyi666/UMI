"""Runs transformer-based return forecasting with optional pretrained factors."""
...
def evaluation(
    model: nn.Module, query: Query, val_range: List[int], loss_fn, device: torch.device
):
    model.eval()
    loss = 0.0
    with torch.no_grad():
...
def evaluation3(
    model_pre_market, model, query, val_range, stk_id_dic, stk_dic_class, device, args
):
    ...
    with torch.no_grad():
        for d in val_range:
            ...
            x_out_all, y__, addi_x, id_out = model_pretrain.stk_gen_normal(
                query, d, stk_id_dic, stk_dic_class
            )
            x = x.to(device)
            y = y.to(device)
            ...
            if args.model_path_pre_market is not None:
                ...
                total_embed, outstks = model_pre_market(
                    x_market.to(device), id_out.to(device), moreout=1
                )
...
def main(
    ...
    if args.pre_type == "market":
        ...
        model_pre_market = model_pretrain.stk_classification_att1(
    ...
    if args.model == "transformer":
        model = model_seq.Trans(
            ...
            add_xdim=len_pre_mask_stk + len_pre_market,
            embeddim=6,
        )
    ...
    if args.use_Adam:
        ...
        model_opt = torch.optim.Adam(
            [
                {"params": model_pre_params, "lr": 1e-7},
        ...
        model_opt = torch.optim.SGD(
            params=model.parameters(), lr=args.learning_rate, momentum=0.9
        )
    ...
    for epoch in range(args.epoch):
        ...
        for d in range(0, len(train_range_other)):
            ...
            x_out_all, y__, addi_x, id_out = model_pretrain.stk_gen_normal(
                query, train_range_other[d], stk_id_dic, stk_dic_class
            )
            x = x.to(device)
            y = y.to(device)
            ...
            loss = torch.nn.functional.mse_loss(scores, y) + 0.1 * ic_loss(
                scores, y_rank, device
            )
            model_opt.zero_grad()
        ...
        ic_va__, rank_ic_va, l1_return_va, l10_return_va = evaluation3(
            model_pre_market,
            model,
            query,
            valid_range,
        ...
        ic_te, rank_ic_te, l1_return_te, l10_return_te = evaluation3(
            model_pre_market,
            model,
            query,
            test_range,
        ...
        ic_va, rank_ic_va, l1_return_va, l10_return_va = evaluation3(
            model_pre_market,
            model,
            query,
            valid_range,
        ...
        if ic_va__ > best_val_loss:
            if args.model_path_pre_market is not None:
                state_ = {
                    "para_pre_market": model_pre_market.state_dict(),
            ...
            torch.save(state_, os.path.join(work_dir, f"best_model.pkl"))
        ...
        print(
            ...
            round(annual_te, 5),
            "annual_va_l10",
            round(annual_va_l10, 5),
            "annual_te_l10",
            round(annual_te_l10, 5),
            "ic_va",
            round(ic_va, 5),
            "ic_te",
            round(ic_te, 5),
